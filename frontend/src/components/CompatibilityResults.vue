<template>
  <div v-if="results && (results.friend_score > 0 || results.coworker_score > 0 || results.partner_score > 0)" class="space-y-4 sm:space-y-6 md:space-y-8 animate-fadeIn">
    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2 sm:gap-3">
          <span class="text-xl sm:text-2xl md:text-3xl">{{ activeCategoryEmoji }}</span>
          <div>
            <h3 class="text-base sm:text-lg md:text-xl font-bold">{{ activeCategoryName }}</h3>
            <p class="text-xs sm:text-sm opacity-90">{{ activeCategorySubtitle }}</p>
          </div>
        </div>
        <div class="flex items-center gap-2 sm:gap-3">
          <div class="flex gap-0.5">
            <span
              v-for="i in 5"
              :key="i"
              class="text-sm sm:text-base transition-all"
              :class="i <= activeCategoryScore ? 'text-amber-200 scale-110' : 'text-white/20'"
            >
              ★
            </span>
          </div>
          <span class="text-sm sm:text-base md:text-lg font-bold bg-white/20 px-2 sm:px-3 py-1 rounded-full">{{ activeCategoryScore }}/5</span>
        </div>
      </div>
    </div>

    <!-- Overall Header -->
    <div ref="overallHeader" class="bg-gradient-to-br from-orange-500 via-orange-600 to-amber-600 rounded-2xl sm:rounded-3xl shadow-2xl p-5 sm:p-7 md:p-9 lg:p-10 text-white border-4 border-orange-400/50 backdrop-blur-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
      ...
      <div 
        ref="friendCard"
        class="category-card group bg-white/90 dark:bg-gray-800/90 rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-5 md:p-6 border-2 border-orange-200/60 dark:border-gray-700/60 hover:border-orange-400 dark:hover:border-orange-600 hover:shadow-3xl transition-all duration-500 hover:-translate-y-1 backdrop-blur-sm"
      ...
      >
        ...
        <div class="space-y-3 sm:space-y-4">
          <template v-if="results.friend_explanation && results.friend_explanation.sections && results.friend_explanation.sections.length > 0">
            ...
            >
              <button
                @click="toggleSection('friend', idx)"
                class="w-full flex items-center justify-between text-left hover:bg-orange-50 dark:hover:bg-gray-700/50 rounded-lg p-2 -m-2 transition-colors"
              >
                <h4 class="text-base sm:text-lg md:text-xl font-bold text-orange-600 dark:text-orange-400 border-b border-orange-200 dark:border-orange-800 pb-1.5 sm:pb-2 flex-1">
                  {{ section.heading || 'Compatibility Analysis' }}
                </h4>
                <svg 
                  class="w-5 h-5 text-orange-600 dark:text-orange-400 transition-transform duration-300 flex-shrink-0 ml-2"
                  :class="expandedSections[`friend-${idx}`] ? 'rotate-180' : ''"
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
              ...
              <div 
                v-show="expandedSections[`friend-${idx}`]"
                class="mt-2 space-y-2 sm:space-y-3 animate-fadeIn"
              >
                <div v-if="section.subcategories && section.subcategories.length > 0" class="space-y-2 sm:space-y-3 ml-1 sm:ml-2">
                  <div
                    v-for="(subcat, subIdx) in section.subcategories"
                    :key="subIdx"
                    class="space-y-1 sm:space-y-1.5"
                  >
                    <h5 class="text-sm sm:text-base md:text-lg font-semibold text-gray-700 dark:text-gray-300">
                      {{ subcat.title }}
                    </h5>
                    <ul class="list-disc list-inside space-y-0.5 sm:space-y-1 ml-2 sm:ml-3">
                      <li
      ...
      >
        ...
        <div class="space-y-3 sm:space-y-4">
          <template v-if="results.coworker_explanation && results.coworker_explanation.sections && results.coworker_explanation.sections.length > 0">
            ...
            >
              <button
                @click="toggleSection('coworker', idx)"
                class="w-full flex items-center justify-between text-left hover:bg-orange-50 dark:hover:bg-gray-700/50 rounded-lg p-2 -m-2 transition-colors"
              >
                <h4 class="text-base sm:text-lg md:text-xl font-bold text-orange-600 dark:text-orange-400 border-b border-orange-200 dark:border-orange-800 pb-1.5 sm:pb-2 flex-1">
                  {{ section.heading || 'Compatibility Analysis' }}
                ...
                <svg 
                  class="w-5 h-5 text-orange-600 dark:text-orange-400 transition-transform duration-300 flex-shrink-0 ml-2"
                  :class="expandedSections[`coworker-${idx}`] ? 'rotate-180' : ''"
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
              ...
              <div 
                v-show="expandedSections[`coworker-${idx}`]"
                class="mt-2 space-y-2 sm:space-y-3 animate-fadeIn"
              >
                <div v-if="section.subcategories && section.subcategories.length > 0" class="space-y-2 sm:space-y-3 ml-1 sm:ml-2">
                  <div
      ...
      >
        ...
        <div class="space-y-3 sm:space-y-4">
          <template v-if="results.partner_explanation && results.partner_explanation.sections && results.partner_explanation.sections.length > 0">
            ...
            >
              <button
                @click="toggleSection('partner', idx)"
                class="w-full flex items-center justify-between text-left hover:bg-orange-50 dark:hover:bg-gray-700/50 rounded-lg p-2 -m-2 transition-colors"
              >
                <h4 class="text-base sm:text-lg md:text-xl font-bold text-orange-600 dark:text-orange-400 border-b border-orange-200 dark:border-orange-800 pb-1.5 sm:pb-2 flex-1">
                  {{ section.heading || 'Compatibility Analysis' }}
                ...
                <svg 
                  class="w-5 h-5 text-orange-600 dark:text-orange-400 transition-transform duration-300 flex-shrink-0 ml-2"
                  :class="expandedSections[`partner-${idx}`] ? 'rotate-180' : ''"
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
              ...
              <div 
                v-show="expandedSections[`partner-${idx}`]"
                class="mt-2 space-y-2 sm:space-y-3 animate-fadeIn"
              >
                <div v-if="section.subcategories && section.subcategories.length > 0" class="space-y-2 sm:space-y-3 ml-1 sm:ml-2">
                  <div
...
const coworkerCard = ref(null)
const partnerCard = ref(null)
const overallHeader = ref(null)
...
watch(() => props.results, (newResults) => {
  if (newResults) {
    ...
    const categories = ['friend', 'coworker', 'partner']
    categories.forEach(category => {
      const explanation = newResults[`${category}_explanation`]
      if (explanation && explanation.sections) {
        explanation.sections.forEach((section, idx) => {
          expandedSections.value[`${category}-${idx}`] = true
        })
      }
    })
  }
...
const activeCategoryName = computed(() => {
  switch(activeCategory.value) {
    ...
    case 'coworker': return 'Workplace'
    case 'partner': return 'Romance'
    default: return 'Compatibility Analysis'
  }
})

const activeCategorySubtitle = computed(() => {
  switch(activeCategory.value) {
    case 'friend': return 'As Friends'
    case 'coworker': return 'As Coworkers'
    case 'partner': return 'As Partners'
    default: return 'Overall Compatibility'
  }
})

const activeCategoryScore = computed(() => {
  if (!props.results || !activeCategory.value) return props.results?.overall_score || 0
  return props.results[`${activeCategory.value}_score`] || 0
})

function toggleSection(category, idx) {
...
onMounted(() => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      ...
      // Only update if we have a visible category with significant visibility
      if (visibleCategory && maxRatio > 0.1) {
        activeCategory.value = visibleCategory
      } else if (maxRatio < 0.1) {
        // Check if we're at the top (overall header visible)
        if (overallHeader.value) {
          const headerRect = overallHeader.value.getBoundingClientRect()
          if (headerRect.top >= 0 && headerRect.bottom > 100) {
  ...
  if (friendCard.value) observer.observe(friendCard.value)
